...
//  预留高度
  ...
  export default {
    ...
    created() {
      // probeType、listenScroll是scroll组件里需要的props
      this.probeType = 3
    ...
    methods: {
      ...
      // 由song-list派发的select事件触发的selectItem（）方法
      selectItem(item, index) {
        this.selectPlay({
          list: this.songs, // 这个list并不是item，而是整个列表
          index: index
        })
      },
      // 点击随机播放
    ...
    watch: {
      // 歌曲列表滚动，添加了一个layer层随着列表滚动
      scrollY(newY) {
        let translateY = Math.max(this.minTranslateY, newY)
        let zIndex = 0
        let scale = 1 // 图片随着列表滚动，放大缩小
        let blur = 0 // 高斯模糊
        this.$refs.layer.style[transform] = `translate3d(0,${translateY}px,0)`
        // 当歌曲列表滚动到最大高度时，给bgImage设置zIndex来遮挡超出的歌曲列表
        const percent = Math.abs(newY / this.imageHeight)
        if (newY > 0) {
          scale = 1 + percent
          // 设置zIndex是不想让列表把图片放大的部分遮住
          zIndex = 10
        } else {
        ...
        } else { // 当列表向下滑动，就列表不在最大高度，需要重置bgImage的zIndex
